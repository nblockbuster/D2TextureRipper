...
std::string getHashFromFile(std::string file)
{
	...
	std::string pkgn = file.substr(0, 4);
	std::string i = file.substr(5, 4);
	std::string two;
	firsthex_int = swapUInt16Endianness(hexStrToUint16(pkgn));
	secondhex_int = swapUInt16Endianness(hexStrToUint16(i));
	one = firsthex_int * 8192;
	two = uint32ToHexStr(one + secondhex_int + 2155872256);
	return two;
}
